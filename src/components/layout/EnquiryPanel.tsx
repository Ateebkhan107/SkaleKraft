      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            <motion.div
              className="fixed bottom-0 right-0 top-0 z-50 w-full overflow-y-auto border-l border-white/10 bg-[#0B0B0B] shadow-2xl md:w-[500px]"
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
            ...
            >
              <div className="flex min-h-full flex-col p-8 md:p-12">
                ...
                <form className="flex flex-1 flex-col gap-6" onSubmit={(e) => { e.preventDefault(); setIsOpen(false); }}>
                  ...
                  <button
                    type="submit"
                    className="mt-8 rounded-full bg-white px-8 py-4 font-medium text-[#0B0B0B] transition duration-300 hover:scale-[0.98] hover:bg-[#805948] hover:text-white"
                  >
                    Send
                  </button>
                </form>
